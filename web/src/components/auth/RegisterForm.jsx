...
const RegisterForm = () => {
  ...
  const [showEmailVerification, setShowEmailVerification] = useState(() => {
    ...
    // 检查是否启用自动 OIDC 重定向--LLM Hub
    const shouldAutoRedirectToOIDC = () => {
        // 检查 URL 参数是否包含 auto_oidc=true
        const autoOidcParam = searchParams.get('auto_oidc');
        if (autoOidcParam === 'true') {
            return true;
        }

        // 检查 localStorage 中是否设置了自动重定向偏好
        const autoOidcPreference = localStorage.getItem('auto_oidc_redirect');
        if (autoOidcPreference === 'true') {
            return true;
        }
        ...
        return status.oidc_enabled && availableRegisterMethods.length === 1;
    };
        ...
        if (
            ...
            setTimeout(() => {
                try {
                    ...
                    console.error('自动 OIDC 重定向失败:', error);
                    setOidcLoading(false);
                    setAutoRedirectTriggered(false);
                    showError(t('自动登录失败，请手动选择注册方式'));
                }
            }, 500);
        }
    }, [status, autoRedirectTriggered, searchParams, t]);
  ...
  const onTelegramLoginClicked = async (response) => {
    ...
    // 如果正在进行自动 OIDC 重定向，显示加载状态--LLM Hub
    if (oidcLoading && autoRedirectTriggered) {
        return (
